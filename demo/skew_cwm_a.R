...
# Maximum skewness: 0.00
# Mixing proportions [equal]: 0.5 and 0.5
#####################################################

# Simulate a mixture model with specified characteristics
set.seed(10)
A <- MixSim.skewcwm(MaxGamma = 0.00, BarOmega = 0.10, K = 2, p = 2)

# Construct a contour plot for the simulated mixture 
contour2d(A, method = "skewcwm", mar = c(1, 1, 1, 1), xlim = c(-4.1, 2), ylim = c(-4.9, 1.9), nlevels = 55, lwd = 1.5, xaxt = "n", yaxt = "n")
