...
#
# Overlap characteristics [high]: Omega = 0.10
# Mixing proportions [unequal]: 0.5 and 0.5
###############################################

# Simulate a mixture model with specified characteristics
set.seed(1)
A <- MixSim.cwm(BarOmega = 0.10, K = 2, p = 2)

# Construct a contour plot for the simulated mixture   
contour2d(A, method = "cwm", mar = c(1, 1, 1, 1), xlim = c(-4.6, 1.5), ylim = c(-9, 23), nlevels = 50, lwd = 1.5, xaxt = "n", yaxt = "n")
